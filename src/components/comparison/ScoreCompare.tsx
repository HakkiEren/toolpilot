...
export function ScoreCompare({ toolA, toolB }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Detailed Breakdown — Semantic <table> for Google featured snippets */}
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <table className="w-full border-collapse" role="table">
          <thead>
            <tr>
              <th scope="col" className="py-3 px-4 bg-gray-50 dark:bg-gray-800 font-semibold text-sm text-gray-500 text-left">Category</th>
              <th scope="col" className="py-3 px-4 bg-blue-50 dark:bg-blue-900/20 font-semibold text-sm text-center text-blue-700 dark:text-blue-400">{toolA.name}</th>
              <th scope="col" className="py-3 px-2 bg-gray-50 dark:bg-gray-800 w-8" aria-hidden="true" />
              <th scope="col" className="py-3 px-4 bg-purple-50 dark:bg-purple-900/20 font-semibold text-sm text-center text-purple-700 dark:text-purple-400">{toolB.name}</th>
            </tr>
          </thead>
          <tbody>
            {ratingCategories.map(({ key, label, icon }) => {
              ...
              return (
                <tr key={key} className={isOverall ? 'font-bold' : ''}>
                  {/* Label */}
                  <td className={`py-4 px-4 border-t border-gray-100 dark:border-gray-800 ${isOverall ? 'bg-gray-50/50 dark:bg-gray-800/30' : ''}`}>
                  ...
                  {/* Tool A Score */}
                  <td className={`py-4 px-4 border-t border-gray-100 dark:border-gray-800 text-center ${isOverall ? 'bg-blue-50/30 dark:bg-blue-900/10' : ''}`} data-value={aScore.toFixed(1)}>
                    <div className="flex items-center gap-2 justify-center">
                      <span className={`text-lg font-bold ${winner === 'a' ? 'text-green-600' : ''}`}>
                        {aScore.toFixed(1)}
                      </span>
                      <div className="w-20 h-2.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden" aria-hidden="true">
                        <div
                  ...
                  {/* VS divider */}
                  <td className={`py-4 px-2 border-t border-gray-100 dark:border-gray-800 text-center ${isOverall ? 'bg-gray-50/50 dark:bg-gray-800/30' : ''}`} aria-hidden="true">
                    {winner === 'tie' ? (
                  ...
                  {/* Tool B Score */}
                  <td className={`py-4 px-4 border-t border-gray-100 dark:border-gray-800 text-center ${isOverall ? 'bg-purple-50/30 dark:bg-purple-900/10' : ''}`} data-value={bScore.toFixed(1)}>
                    <div className="flex items-center gap-2 justify-center">
